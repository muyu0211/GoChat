...
type MessageModel struct {
	...
	ConversationID string `gorm:"type:varchar(32);index:idx_conversation_id;not null;comment:会话id，使用会话双方id得到"`
	// ClientMsgID    string `gorm:"type:varchar(64);"`          // 客户端生成的UUID (用于去重)
	SeqID      uint64 `gorm:"index:idx_conversation_seq"` // 会话内序列号 (严格递增)
